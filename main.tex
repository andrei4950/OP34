...
There are four main factors that limit the maximum wavenumber measurement resolution $\delta \nu$ that can be obtained: Doppler broadening \ref{item: doppler}, Airy distribution \ref{item: resolution}, the non-uniformity of the B-field \ref{item: field} and the mirror flatness \ref{item: flattness}. 
...
\subsection{Measurement of the line splitting $\Delta \nu$} \label{sec: measuremet}
...
We first studied the $5^1 D_2\rightarrow 5^1 P_1$ transition for the cadmium red line. We used the quantum numbers of the initial and final states (Table \ref{tab: quantum_numbers}) and equation \eqref{eq: g eff} to find the possible values for the effective g-factor $g_{eff} = -1, 0, 1$. This means that we expected the line to split into 3 components, with the splitting given by equation \eqref{eq: splitting}. The theoretical value of the linear line splitting $\Delta \nu $ as function of magnetic field $B$ for $g_{eff} = \pm 1$ is: 
\begin{equation}
...
\begin{wrapfigure}{r}{0.4\textwidth}
    \centering
    \includegraphics[width=\linewidth]{polarisation.png}
    \caption{Polarisation of the sigma lines. The outer fringe is RH polarised and the inner fringe is LH polarised}
...
%\begin{figure}[h!]
%    \centering
%    \includegraphics[width=0.4\linewidth]{polarisation.png}
%    \captionsetup{justification=centering}
...
\begin{table}[h!]
  ...
  \end{tabular}
    \captionsetup{justification=centering}
  \caption{Cadmium green line: measured line splitting wavenumber $\Delta \nu$ dependence on magnetic field $B$ and $g_{eff}$ factor in the $5^3 S_1\rightarrow 5^3 P_2$ transition. The values of \(g_{eff}\) are shown with the corresponding polarisation}
...
We also attempted to measure the inherent instrumental resolution by making use of a He-Ne laser, with a much narrower line \cite{henelaser}. The laser light line width was measured using a Czerny-Turner monochromator and determined to be $\lambda_{\text{He-Ne}} = 633.1 \pm 0.4 \si{nm}$. We measured the FWHM of the peaks in the line profile (Fig. \ref{img: He-Ne line profile}) of the Fabry-Perot interference image (Fig. \ref{img: He-Ne image}) and obtained a resolution limit of:
\begin{equation}
...
\end{equation}
This suggests that the flatness of the mirror (item \ref{item: flattness} from section \ref{sec: instr err}) accounts for most of the error in our measurement. The low instrumental resolution will be dominant over the Doppler broadening (equation \eqref{eq: nu doppler}).
